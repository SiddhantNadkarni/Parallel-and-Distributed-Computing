...
void* K_MeansMultithreaded(void* threadP)
{
	...
	for (int iters = 0; iters < 50; ++iters)
	{
		...
		//lock variable and check for conditional variable
		pthread_mutex_lock(&r_lock);
		if (r < p-1)  //if r is not the last thread, put thread to sleep
		{
			r++; 
			pthread_cond_wait(&r_cv, &r_lock);
		}
		else  //calculate mean if all threads have finished clustering
		{
		
			for (int i = 0; i < 6; ++i) //calculates mean 
			{
				int sumAll(0);
				int lengthAll(0);
				for (int j = 0; j < p; ++j)
			...
			pthread_cond_broadcast(&r_cv); //wake up threads
		}
...
int main(int argc, char const *argv[])
{
	...
	unsigned char* Image1D = readImage("input.raw", h, w, 1); 
	...
	for (int i = 0; i < p; ++i)
	{
		...
		threadT[i].sum = &sum;
		threadT[i].no = &no;
		threadT[i].numThreads = p;
		threadT[i].means = &means;
